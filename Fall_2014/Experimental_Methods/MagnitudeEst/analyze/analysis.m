...
% Summary stats
stats.inM = mean(mean(inMean));
stats.inSEM = mean(mean(inSD))/sqrt(n);
stats.outM = mean(mean(outMean));
stats.outSEM = mean(mean(outSD))/sqrt(n);
[h,stats.p] = ttest2(inMean, outMean);


close all
ax = figure(1);
hold on
plotM = [stats.inM stats.outM];
plotSEM = [stats.inSEM stats.outSEM];
plotCol = [1 0 0; 0 0 1];
he = errorbar(plotM,plotSEM,'k.');
for i = 1:length(plotM)
    plot(i, plotM(i), '.', 'color', plotCol(i,:), 'markersize', 30);
end
set(gca, 'XTick', 1:2, 'XTickLabel', {'>-<', '<->'});
title('Muller-Lyer Illusion - All Conditions');
