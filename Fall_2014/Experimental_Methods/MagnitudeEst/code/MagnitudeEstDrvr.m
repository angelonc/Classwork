...
try
    ...
    for i = 1:params.nBlocks
        ...
        for k = 1:nDataRows
            % Precompute lines for each trial, with jitter if specified
            if jitter(i)
                thisAngle = theData(k,2)+(randi(91,1)-46);
            else
                thisAngle = 180;
            end
            lineInfos(k) = PrecomputeLineME(theData(k,1), thisAngle, theData(k,3), theData(k,4)); %#ok<AGROW>
        end
    ...
    % exists.
    dataFile = sprintf('%s/%s-%d-SJ%d.csv', dataFolder, params.experimentName, GetNextDataFileNumber(dataFolder, '.csv'), params.startJitter);
