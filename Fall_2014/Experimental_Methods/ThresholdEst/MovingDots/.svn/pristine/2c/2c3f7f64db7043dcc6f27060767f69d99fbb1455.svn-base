...
% around.
% Biasing lower values logarithmically:
logCoh = logspace(-1,0,5);
params.test.dotCoherence = [fliplr(logCoh)*-1 0 logCoh];
%params.test.dotCoherence = [-1 -0.8 -0.6 -0.4 -0.2 0 0.2 0.4 0.6 0.8 1]; % original: [-1 -0.6 -0.3 -0.15, 0 0.15 0.3 0.6 1];
...
params.test.HorV = 1;					% Direction of dot motion, 0=left/right, 1=up/down
...
% Sound parameters:
%
params.sound.freqRange = [500 1000];    % Frequency shift range
params.sound.duration = .5;              % Sound duration in seconds
params.sound.samplingRate = 1000;
params.sound.direction = [-1 0 1];       % -1 = down, 1 = up, 0 = constant
...
params.trialDuration = .5;				% Trial duration (seconds)
...
% Key mappings - these are TOTALLY wrong...
...
params.subject = 5;                     % Name of the subject.
